  ...
  apps.funkwhale = {
    ...
    services.runtimes.nixos = {
      ...
      nixosConfig = {
        ...
        systemd.services.funkwhale-server = {
          ...
          environment = lib.mkForce globalEnv;
          serviceConfig = {
        ...
        systemd.services.funkwhale-worker = {
          ...
          serviceConfig = {
            ...
            ExecStart = "${pkgs.writeShellScript "funkwhale-worker-init" ''
              export DATA_DIR="/var/lib/funkwhale"
              export PATH=$PATH:${pkgs.coreutils}/bin:${pkgs.ffmpeg}/bin
    ...
    test.services = {
      ...
      script = ''
        ...
        if $curl http://localhost:5000 | grep -iq "funkwhale"; then
        ...
        if $curl -I -L http://localhost:5000/manifest.json | grep -Ei "application/(manifest\+)?json"; then
